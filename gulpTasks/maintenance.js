...
gulp.task('update-nodes', (cb) => {
    ...
    .then((tagName) => {
        const latestPirlVersion = tagName.match(/\d+\.\d+\.\d+/)[0];

        // Compare to current pirl version in clientBinaries.json
        if (cmp(latestPirlVersion, localPirlVersion)) {
            pirl.version = latestPirlVersion;

            // Query commit hash (first 8 characters)
            got(`https://api.github.com/repos/pirl/pirl/commits/${tagName}`, { json: true })
            .then((response) => {
            ...
            .then((hash) => {
                ...
                .then((response) => {
                    ...
                    _.keys(pirl.platforms).forEach((platform) => {
                        _.keys(pirl.platforms[platform]).forEach((arch) => {
                            // Update URL
                            let url = pirl.platforms[platform][arch].download.url;
                            url = url.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestPirlVersion}-${hash}`);
                            pirl.platforms[platform][arch].download.url = url;

                            // Update bin name (path in archive)
                            let bin = pirl.platforms[platform][arch].download.bin;
                            bin = bin.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestPirlVersion}-${hash}`);
                            pirl.platforms[platform][arch].download.bin = bin;

                            // Update expected sanity-command version output
                            pirl.platforms[platform][arch].commands.sanity.output[1] =
                            String(latestPirlVersion);

                            // Update md5 checksum
                            blobs.forEach((blob) => {
                                if (String(blob.Name) === _.last(pirl.platforms[platform][arch].download.url.split('/'))) {
                                    const sum = new Buffer(blob.Properties[0]['Content-MD5'][0], 'base64');

                                    pirl.platforms[platform][arch].download.md5 = sum.toString('hex');
                                }
                            });
                        });
                    });
