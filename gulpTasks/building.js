...
const applicationName = (options.wallet) ? 'Pirl Wallet' : 'Mist';
...
gulp.task('bundling-interface', (cb) => {
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            console.log('Use local wallet at ../Oystr-Wallet-dApp/app');
            bundle(`&& cd ../../meteor-dapp-wallet/app \
                && meteor-build-client ../../mist/dist_${type}/app/interface/wallet -p ""`);
        } else {
            console.log(`Pulling https://github.com/pirl/Oystr-Wallet-dApp/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/pirl/Oystr-Wallet-dApp.git \
                && cd Oystr-Wallet-dApp/app \
                && meteor-build-client ../../app/interface/wallet -p "" \
                && cd ../../ \
                && rm -rf Oystr-Wallet-dApp`);
        }
    } else {
        bundle();
    }
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/pirl/mist',
        build: {
            appId: `com.pirl.${type}`,
            asar: true,
            directories: {
